...
print(f'\tVypíše každý sudý prvek ze seznamu letters: {letters[::2]}') # Podle sudého indexu, tzn. 0, 2, 4
...
# V tomto případě budou osoby seřazeny podle jmen reverzně (sestupně) od nejmenšího po největší
...
gen = [*range(1,2001,1)]
hundreds = list(filter(lambda item: item%200==0, gen))
...
del ascii[-len(hundreds):] # zkrácení od konce, aby vznikla stejná velikost seznamu podle hundreds
...
persons.append(tuple(('Magdaléna', 30, 'žena')))
persons.append(tuple(('Jan', 32, 'muž')))
persons.append(tuple(('Simona', 40, 'žena')))
print('Aktuální seznam persons\n',persons)
...
print("--------------------")
for o in ipeople:
    print(f'{i};{o[0]};{o[1]};{o[2]}')
    i+=1

print("\nCSV struktura i-lidí pomocí comprehensions")
print("--------------------")
ipeople = [item for item in persons if "i" in item[0].lower()]
ipeople.sort(key = sort_item, reverse=True)
for o in ipeople:
    print(f'{i};{o[0]};{o[1]};{o[2]}')
    i+=1
