...
male = re.compile(r'_([a-z])')
def identifikator_variable(retezec):
    a = retezec.lower().replace(" ", "_")
    b = male.sub(lambda x: x.group(1).upper(), a)
    return print(f"{a}\n{b}")

identifikator_variable("Prave se ucim pracovat s Pythonem")

import string
import random
import itertools
def tvorba_hesla(osoby):
    for _ in itertools.repeat(None, osoby):
        pismeno_velke = string.ascii_uppercase
        pismeno_male = string.ascii_lowercase
        specialni = string.punctuation #specialni znaky z punctuation
        numbers = string.digits
        result_pismeno_velke = ''.join(random.choice(pismeno_velke) for i in range(3))
        result_pismeno_male = ''.join(random.choice(pismeno_male) for i in range(3))
        result_specialni = ''.join(random.choice(specialni) for i in range(1))
        result_numbers = ''.join(random.choice(numbers) for i in range(3))
        print(result_pismeno_velke + result_pismeno_male + result_specialni + result_numbers)
